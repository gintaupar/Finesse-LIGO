...
x_symbols = (lho.SRM.Rcx.ref, lho.SR3.Rcx.ref, lho.SR2.Rcx.ref, lho.ls1.L.ref, lho.ls2.L.ref)
y_symbols = (lho.SRM.Rcy.ref, lho.SR3.Rcy.ref, lho.SR2.Rcy.ref, lho.ls1.L.ref, lho.ls2.L.ref)

SRX_RT_GOUYx = generate('x', x_symbols)
SRX_RT_GOUYy = generate('y', y_symbols)
...
# %%
samples_gouy_x = SRX_RT_GOUYx(samples).squeeze()
samples_gouy_y = SRX_RT_GOUYy(samples).squeeze()
...
idx = np.bitwise_and(
    ...
    result[-2:].mean(0) > 19.*2,
    result[-2:].mean(0) < 19.8*2,
)
# %%
import corner
figure = corner.corner(
    ...
    labels=[
        ...
        r"$Rc SR3$",
        r"$Rc SR2$",
        r"$L, SRM \rightarrow 2$",
        r"$L, SR2 \rightarrow 3$",
        r"X",
        r"Y",
    ],
...
plt.savefig("./figures/cold_state_mcmc_src.pdf")
# %%
means = result.mean(1)
stds = result.std(1)
