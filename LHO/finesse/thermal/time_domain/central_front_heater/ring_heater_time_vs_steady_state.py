...
# %%
ts_itmx = AdvancedLIGOTestMass3DTime(model, 3600//2)
ts_itmx.t = 0
ts_itmx.set_initial_condition(initial_condition)
...
while ts_itmx.t <= 3600*26:
    if ts_itmx.t >= 5*3600:
        ts_itmx.dt.value = 3600
    print(ts_itmx.t)
    ts_itmx.step()
    t2.append(ts_itmx.t)
    full_HR_surf.append(get_deformation(x, y, ts_itmx))
...
def get_dioptres(t, HR_surf):
    ...
    for i, _t in enumerate(t):
        ...
        rx, ry = mmap.get_radius_of_curvature_reflection(53e-3)
...
def get_substrate_dioptres(t, OPD):
    ...
    for i, _t in enumerate(t):
        ...
        rx, ry = mmap.get_thin_lens_f(53e-3)
...
plt.plot(t/3600 + 26, surface_diopters / 1e-6, c='b')
plt.scatter(t/3600 + 26, surface_diopters / 1e-6, c='b', label='Start from steady state')
...
plt.plot(t/3600+26,  substrate_diopters/1e-6, c='r')
plt.scatter(t/3600+26,  substrate_diopters/1e-6, c='r', label='Start from steady state')
