...
@pytest.fixture
def ppath_join():
    """Fixture that joins subpaths to the yaml parameter directories for
    each IFO

    Example
    -------
    ppath_join('LHO', 'lho_O4.yaml')
    """
    import git
...
def generate_figs():
    ...
    def func(*keys):
        def make_fig():
            fig, ax = subplots()
            return ax
        return Munch({k: make_fig() for k in keys})
